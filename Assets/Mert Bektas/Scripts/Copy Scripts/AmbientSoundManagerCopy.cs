...
public class AmbientSoundManagerCopy : MonoBehaviour
{
    ...
    IEnumerator PlayRandomSoundLoop(AudioSource source, AudioClip[] clips, float minDelay, float maxDelay)
    {
        ...
        while (true)
        {
            ...
            if (!source.enabled)
            {
                Debug.LogWarning($"{source.name} AudioSource kapal�yd�, yeniden a��ld�.");
                source.enabled = true;
            }

            if (!source.gameObject.activeInHierarchy)
            {
                Debug.LogWarning($"{source.name} GameObject inactive, ses �al�namad�.");
                continue;
            }
